...
def test_image_file_validate_schema_response():
    payload = json.dumps({
        "image_file_path": f"{path_helper.get_resource_path(ApiImageResourcesData.karaburma_main_image)}",
        "type_element": "button",
        "is_read_text": False
    ...
    response_json = json_helper.convert_text_into_json(response.text)
    validator = Validator(SCHEMA_RESPONSE_FILE_MODE_BUTTON_ELEMENTS_ONLY, require_all=False)
